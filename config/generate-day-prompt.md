...
Для кожного запису з виводу скрипта (є `place`, незалежно від того, вдалося створити
подію в календарі чи ні) одразу запиши час і в саму таску, щоб бот міг нагадати про неї
за 15 хв до початку:

```sh
curl -s -X PATCH -H "x-api-token: $TOKEN" -H "Content-Type: application/json" \
  "http://127.0.0.1:3464/schedule-tracker-api/day-items/<id>" \
  -d '{"due_at":"<ISO start, той самий що йшов у create_event>"}'
```

`<id>` — те саме `id` з виводу скрипта (крок 2.3), `due_at` — ISO-час початку з тим самим
Europe/Kyiv offset, що рахував для `create_event`. Це окремо від календарних подій:
day_items з due_at отримують пуш у Telegram за 15 хв до `due_at` (окремий cron у боті,
не плутати з `meetings-poll`). Таски, для яких місця не знайшлось (start == null у
кроці 2.3) — due_at не виставляй, вони лишаються без нагадування.
...
2. Зібрати `data` — обʼєкт під усі токени. Обовʼязкові поля:
   ...
   - див. «Зміст блоків» нижче для `magic_card`/`magic_fact`/`magic_prophecy`/`magic_absurd`
     (усі чотири — звичайні поля `data`, жодного `{% %}`), `space`, `news`, `joke_diagram`,
     `spice`, `moon`, `stats`, `claude_comments` (масив), `header_image`, `image_anchor`.
...
Віддай МАСИВ `claude_comments` — 5-7 реплік РІЗНОЇ ДОВЖИНИ (від однієї фрази до двох
речень). Кожна репліка — БЕЗ префікса `•Коментар клода•` і без `>`/`<aside>` (обгортку і
формат додає рушій сам). Це не резюме блоку, а кинута збоку ремарка: спостереження,
підкол, зауваження про самого Марка. У розкладку по слотах НЕ лізь: рушій (`template.js`)
детерміновано від дати сам обере 2-4 з наявних `{? claude_comment ?}`-слотів і роздасть у
них РІЗНІ репліки, решту слотів прибере. Скалярний `claude_comment` більше не потрібен —
передавай саме масив.
